...
class TransformerBackend(ModuleBackend):
    ...
    def get_inference_cache_descriptors(self, batch_size: int, max_length: int) -> Sequence[TensorDescriptor]:
        ...
        for device, num_heads in zip(self.module.devices, self.shard_num_heads):
            num_heads //= self.block_config.num_key_value_groups
    ...
    def _select_layer_past(self, cache_tensors: Sequence[torch.Tensor], prefix_length: int) -> Sequence[torch.Tensor]:
        ...
        for i in range(len(key_cache)):
            key_cache[i] = key_cache[i].flatten(0, 1)[:, :, :prefix_length]  # [batch * num_kv_heads, head_dim, kv_length]
            value_cache[i] = value_cache[i].flatten(0, 1)[:, :prefix_length]  # [batch * num_kv_heads, kv_length, head_dim]
