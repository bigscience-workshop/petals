...
class DistributedBloomConfig(BloomConfig):
    ...
    sequence_manager_timeout: int = 20  # a number of seconds for waiting result from each node
...
class DistributedBloomModel(BloomModel):
    ...
    def __init__(self, config: DistributedBloomConfig):
        ...
        self.h = RemoteSequential(
            config, dht, config.dht_prefix, sequence_manager_timeout=config.sequence_manager_timeout
        )
